...
import shiroi.top.service.HuYaService
...
class HuYa {
    ...
    private var headPattern = "<img id=['\"]avatar-img['\"] src=([\"'])?(?<src>[^'\"]+)[^>]*>"

    private var namePattern = "<img id=['\"]avatar-img['\"].*alt=([\"'])?(?<alt>[^'\"]+)[^>]*>"

    private var liveStatusPattern = "['\"]stream['\"]: null"

    private val huYaService = HuYaService.create
     ...
     * @return HuYa
     */
    fun create(id: String): HuYa {
        //设置id
        setId(id.trim())
        //数据请求
        val h = huYaService.getId(id.trim()).execute().body().string()
        if(h.isNotEmpty()) {
            setHtml(h)
            initParam()
        }
    ...
    override fun toString(): String {
        return "HuYa(name='$name', id='$id', head='$head', liveStatus=$liveStatus)"
    }
    ...
    class Config{
        ...
        override fun toString(): String {
            return "HuYa(isOpen=$isOpen, group=${group.contentToString()}, listen=$listen)"
        }

        //初始化
        fun load() {
            val fileName = Plugin.configFolder.toString() + "\\" + "huya.json"
                            ...
                            Plugin.launch {
                                bot.groups.forEach {
                                    if(("*" in configObject.group) || (it.id.toString() in configObject.group)) {
                                        //保存的数据
                                        val fileName = Plugin.dataFolder.toString() + "\\data\\huya\\${it.id}.json"
                                        //初始化文件
                                        if(FileUtil().fileExists(fileName)) {
                                            //主播列表
                                            var list = Gson().fromJson(FileUtil().openFile(fileName), HuYa().dataList::class.java)
                                            //获取所有keys
                                            var live = list.keys
                                            live.forEach { user ->
                                                //定义tmp文件
                                                var tmp = Plugin.dataFolder.toString() + "\\tmp\\huya-${it.id}-${user}.tmp"
                                                //获取主播信息
                                                var body = HuYa().create(user)
                                                //直播中
                                                if(body.getLiveStatus()) {
                                                    if(!FileUtil().fileExists(tmp)) {
                                                        FileUtil().saveFile(tmp,user)
                                                        it.sendMessage("name:${body.getName()},id:${user},head=${body.getHead()}\n${HuYaService.base_url}${user}\n正在直播中~")
                                                    }
                                                } else {
    ...
    //权限名 `/permission permit * shiroi.top.plugin:command.huya:*`
    object Command: CompositeCommand(
        ...
        suspend fun CommandSender.list() {
            ...
            if (group is Group) {
                ...
                var strMessage = ""
                //保存的数据
                val fileName = Plugin.dataFolder.toString() + "\\data\\huya\\${group.id}.json"
                //初始化文件
                if(FileUtil().fileExists(fileName)) {
                    var list = Gson().fromJson(FileUtil().openFile(fileName), HuYa().dataList::class.java)
                    //遍历数据
        ...
        suspend fun CommandSender.add(id: String) {
            ...
            if (group is Group) {
                ...
                val fileName = Plugin.dataFolder.toString() + "\\data\\huya\\${group.id}.json"
                //获取主播信息
                val body = HuYa().create(id)
                //是否存在
                if(body.getExists()) {
                    var list = HuYa().dataList
                    //初始化文件
                    if(!FileUtil().fileExists(fileName)) {
                        ...
                        list = Gson().fromJson(FileUtil().openFile(fileName), HuYa().dataList::class.java)
                        list[id] = Data(body.getName(), body.getId(), body.getHead())
                        FileUtil().saveFile(
                            fileName
                            ,JsonUtil().toJsonParse(Gson().toJson(list))
